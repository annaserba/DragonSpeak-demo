...
`src/shared/api/mock-websocket/mockQuestSocket.ts` симулирует server behavior:
...
- `entities` хранят domain data models.
- `shared` содержит reusable infrastructure и game engine.

## 3D Scene

`src/widgets/quest-scene/RestaurantThreeScene.tsx` реализует легкую интерактивную Three.js сцену ресторана:
